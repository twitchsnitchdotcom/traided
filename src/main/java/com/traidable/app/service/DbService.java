...
public class DbService {
    ...
    public void addDBConstraints() {
        ...
        ResultSummary submissionPeriodCompositeConstraint = PersistenceService.getClient().query("CREATE CONSTRAINT submission_period_composite ON (s:SubmissionPeriod) ASSERT (s.submission_fiscal_year, s.submission_fiscal_quarter, s.submission_fiscal_month) IS NODE KEY;").in(database).run();
        ResultSummary subAgencySummaryCompositeConstraint = PersistenceService.getClient().query("CREATE CONSTRAINT subagency_summary_composite ON (s:SubAgencySummary) ASSERT (s.toptier_code, s.fiscal_year) IS NODE KEY;").in(database).run();
